"""Create thumbnail grids or per-slide JPEGs from PowerPoint presentations.

Grid mode (QA): layout of slide thumbnails for quick visual analysis.
Slides-dir mode (browser preview): numbered slide-01.jpg, slide-02.jpg, ...

Usage:
    python thumbnail.py input.pptx [output_prefix] [--cols N]
    python thumbnail.py input.pptx --slides-dir public/screenshots/powerpoint/my-deck [--dpi 150]

Examples:
    python thumbnail.py presentation.pptx
    # Creates: thumbnails.jpg

    python thumbnail.py deck.pptx qc/grid --cols 3
    # Creates: qc/grid.jpg (agent QA)

    python thumbnail.py deck.pptx --slides-dir public/screenshots/powerpoint/claude-code --dpi 150
    # Creates: slide-01.jpg, slide-02.jpg, ... (browser slideshow)
"""
...
def resolve_pdftoppm() -> str:
    override = os.environ.get("PDFTOPPM_PATH", "").strip()
...
def main():
    ...
    parser.add_argument(
        "--slides-dir",
        help="Export numbered slide-NN.jpg files to this directory (browser preview)",
    )
    parser.add_argument(
        "--dpi",
        type=int,
        default=150,
        help="Resolution for PDF to JPEG conversion (default: 150, grid mode uses 100)",
    )
    ...
    try:
        if args.slides_dir:
            count = export_slides_to_dir(input_path, Path(args.slides_dir), dpi=args.dpi)
            print(f"Exported {count} slide image(s) to {args.slides_dir}")
            return
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            visible_images = convert_to_images(input_path, temp_path, dpi=CONVERSION_DPI)
...
def export_slides_to_dir(pptx_path: Path, out_dir: Path, dpi: int = 150) -> int:
    out_dir.mkdir(parents=True, exist_ok=True)
    slide_info = get_slide_info(pptx_path)

    with tempfile.TemporaryDirectory() as temp_dir:
        temp_path = Path(temp_dir)
        visible_images = convert_to_images(pptx_path, temp_path, dpi=dpi)
        ...
        for i, (img_path, _) in enumerate(slides, start=1):
            dest = out_dir / f"slide-{i:02d}.jpg"
...
def convert_to_images(pptx_path: Path, temp_dir: Path, dpi: int = CONVERSION_DPI) -> list[Path]:
    ...
    if result.returncode != 0 or not pdf_path.exists():
        ...
        raise RuntimeError(f"PDF conversion failed{': ' + detail if detail else ''}")
