...
6. **Open the capture URL in the browser.** Show the user the full URL first, then open it. Include `&figmadelay=2000` so the page has time to render. Format:
   ```
   http://localhost:5173/?mode=<mode-key>#figmacapture=<captureId>&figmaendpoint=<encoded-endpoint>&figmadelay=2000
   ```
   **Always show the user the full URL before opening it.**
