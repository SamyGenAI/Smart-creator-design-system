/**
 * Infographic — 1080×1350px LinkedIn infographic template.
 * Mirrors the "The ultimate Claude guide" layout from Figma exactly.
 * Figma node: 5:15871
 *
 * LAYOUT (top to bottom, inner width 981px):
 *   Header   — large title + italic subtitle (flex-1 grows to fill remaining space)
 *   Row 1    — 2 cards: Card1 (321px) + Card2 (633px), each with a glass-navy-section
 *   Row 2    — full-width card (977px) with icon-bullet, table, optional image
 *   Row 3    — 3 equal cards (321px + 308px + 308px) with glass-navy-sections
 *   Row 4    — full-width card (981px) with pastel card + two checklists
 *   Footer   — "Follow for more" pill bar
 *
 * DATA PROP SHAPE  (see src/App.jsx for a complete example):
 * {
 *   title:    string,
 *   subtitle: string,
 *   row1: {
 *     card1: { title, iconSrc, checklist: { title, items[] } },
 *     card2: { title, iconSrc, iconBullet: { items[] }, imageSrc }
 *   },
 *   row2: { title, iconSrc, iconBullet: { items[] }, table: { headers[], rows[][] }, imageSrc },
 *   row3: {
 *     card1: { title, iconSrc, sectionTitle, numberBullet: { items[] } },
 *     card2: { title, iconSrc, sectionTitle, logos[] },
 *     card3: { title, iconSrc, sectionTitle, highlightText, colorBoxColor }
 *   },
 *   row4: {
 *     title, iconSrc, sectionTitle,
 *     pastelCard: { text },
 *     checklist1: { title, items[] },
 *     checklist2: { items[] }
 *   },
    ...
    >
      ...
      >
        ...
        >
          ...
          {/* Card 3 — Colored text boxes + highlight */}
          <div
          ...
          >
            ...
            />
            {/* Text boxes beside the color grid */}
            <TextBox
              text=""
