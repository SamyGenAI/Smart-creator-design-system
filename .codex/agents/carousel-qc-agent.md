...
6. **Cover slide has:** bold title (≤50 chars, fontSize ≥ 100px), subtitle text, an illustration `<img>`, NO AccentPill, NO colored rectangle behind text
7. **Step slides have:** `StepLabel` or equivalent numbered prefix. `ScreenPlaceholder` is only required if the brief or user explicitly requested it.
